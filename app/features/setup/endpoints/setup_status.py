...
def update_profile(request: ProfileUpdateRequest, db: Session = Depends(get_db)):
    ...
    if request.user_type == "cast" and request.cast_type:
        # 'A' or 'B' を直接セットできるように
        user.cast_type = request.cast_type if request.cast_type in ["A", "B"] else ("A" if request.cast_type == "cas" else "B")
    db.commit()
    ...
    if cast_profile:
        ...
        # キャストタイプを設定（A/B直接受け入れ）
        if request.cast_type:
            cast_profile.cast_type = request.cast_type if request.cast_type in ["A", "B"] else ("A" if request.cast_type == "cas" else "B")
    else:
        # 新規作成 (rank_id を 1 に設定)
        try:
            cast_profile = CastCommonProf(
                cast_id=request.user_id,
                cast_type=request.cast_type if request.cast_type in ["A", "B"] else ("A" if not request.cast_type or request.cast_type == "cas" else "B"),
                rank_id=1,
